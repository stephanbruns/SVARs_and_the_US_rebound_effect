# I added the possibility of having exogenous variables in the VAR estimation
#'Wild bootstrap for IRFs of identified SVARs (31.1.2020)
#'
...
wild.boot_mod_exgvar <- function(x, rademacher = TRUE, n.ahead = 20, nboot = 500, nc = 1, dd = NULL, signrest = NULL, itermax = 300, steptol = 200, iter2 = 50, exog_varb = NULL, Bex=NULL){
  ...
  u <- t(y[-c(1:p),]) - A %*% Z
  if (!is.null(exog_varb)){
    Xg<-exog_varb[-c(1:p),]
    u <- t(y[-c(1:p),]) - A %*% Z - Bex %*% t(Xg)
  }
  ...
  bootf <- function(Ustar1){
    ...
    if (!is.null(exog_varb)){
      Ystar <- t(A %*% Z + Bex %*% t(exog_varb[-c(1:p),]) + Ustar1)
        }
    
    Bstar <- t(Ystar) %*% t(Z) %*% solve(Z %*% t(Z))
    if (!is.null(exog_varb)){
      ZXg<-rbind(Z,t(Xg)) # this matrix is 12 x 295
      Bstar_exg <- t(Ystar) %*% t(ZXg) %*% solve(ZXg %*% t(ZXg)) # dim: 3 x 12
    }
   
    Ustar <- Ystar - t(Bstar %*% Z)
    if (!is.null(exog_varb)){
     Ustar <- Ystar - t(Bstar_exg %*% ZXg)
     Bstar<-Bstar_exg[,-c((ncol(Bstar_exg)-1), ncol(Bstar_exg))] ## if you have more than two exogenous variables change here
     }
    ...
    }else if(x$method == "fastica"){
      temp <- id.fastica(varb)
    }else{
    ...
    if(!is.null(temp)){
      ...
      ip <- vars::irf(temp, n.ahead = n.ahead)
