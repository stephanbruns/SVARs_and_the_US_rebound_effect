...
#	fastICA
#	5 variables SVAR (energy-gdp-prices-ind.prod-e.quality) 
#
#-------------------------------------------------------------------------
...
set.seed(42) #for exact reproducibility of the results
...
aba[cc3[1],]<-0
aba[,cc3[2]]<-0
cc4<-which(aba == max(aba), arr.ind = TRUE)
aba[cc4[1],]<-0
aba[,cc4[2]]<-0
cc5<-which(aba == max(aba), arr.ind = TRUE)
CC<-rbind(cc1,cc2,cc3,cc4, cc5)
e_s<-CC[CC[,1]==1,2] #energyshock
y_s<-CC[CC[,1]==2,2] #gdpshock
p_s<-CC[CC[,1]==3,2] #priceshock
i_s<-CC[CC[,1]==4,2] #i-shock
q_s<-CC[CC[,1]==5,2] #q-shock
perme<-c(e_s, y_s, p_s, i_s, q_s)
Ap<-Ascaled[,perme]
colnames(Ap) <-c("e","s2","s3", "s4", "s5")
...
### fastICA estimation
iden<-id.fastica(VAR.est, sseed=42)
Ap<-fAp(ures)
iden$B<-Ap # this is the mixing matrix
trueW<-solve(Ap) # this is the unmixing matrix 
trueeres<-t(trueW %*% t(ures)) # these are the independent components (shocks)

round(iden$B,2) #Mixing matrix of fastICA (Table 6) (confidence intervals are calculated below)
#
...
round(c.i.,2) #### Confidence intervals of fastICA 

#save data
res <- list("uniB.fastica_Q.v5.p3.levels", iden, boots)
save(res, file=paste(savepath, "irf.fastica_Q.v5.p3.levels.iter1000.RDATA", sep=""))
...
Ap<-fAp(ures)
iden$B<-Ap # this is the mixing matrix
round(iden$B,2) #Mixing matrix of fastICA (Table 5) (confidence intervals are calculated below in the rebound function)
...
#save data
res <- list("uniB.fastica_M.v5.p3.levels", iden, boots)
save(res, file=paste(savepath, "irf.fastica_M.v5.p3.levels.iter1000.RDATA", sep=""))
...
filename <- c("irf.fastica_Q.v5.p3.levels.iter1000.RDATA",
              "irf.fastica_M.v5.p3.levels.iter1000.RDATA")
