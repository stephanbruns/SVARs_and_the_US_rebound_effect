...
VAR.est <- vars::VAR(D, p = 3, type="const")
...
round(iden$B,2) #Mixing matrix of fastICA in Table 2 (confidence intervals are calculated below)
#
...
round(c.i.,2) #### Confidence intervals of fastICA in Table 2

#save data
res <- list("uniB.fastica_Q.v3.p3.levels", iden, boots)
save(res, file=paste(savepath, "irf.fastica_Q.v3.p3.levels.iter1000.RDATA", sep=""))
...
# Bootstrap
boots <- wild.boot_mod(iden, rademacher = T, nboot = nbs, n.ahead = hori_Q)
res <- list("uniB.fastica_Q.v3.p3.levels.iter100_startin1992", iden, boots)

save(res, file=paste(savepath, "irf.fastica_Q.v3.p3.levels.iter1000_startin1992.RDATA", sep=""))
...
# confidence intervals
boots <- wild.boot_mod(iden, rademacher = T, nboot = nbs, n.ahead = hori_M)
q10 <- function(x) {quantile(x, probs=c(0.05,0.95))}
contemp_sig <- matrix(ncol = ncol(boots$bootstrap[[1]]$irf), nrow = length(boots$bootstrap))
colnames(contemp_sig) <- colnames(boots$bootstrap[[1]]$irf)
for (j in 1:length(boots$bootstrap)) {
...
#save data
res <- list("uniB.fastica_M.v3.p3.levels", iden, boots)
save(res, file=paste(savepath, "irf.fastica_M.v3.p3.levels.iter1000.RDATA", sep=""))
...
filename <- c("irf.fastica_Q.v3.p3.levels.iter1000.RDATA",
              "irf.fastica_Q.v3.p3.levels.iter1000_startin1992.RDATA",
              "irf.fastica_M.v3.p3.levels.iter1000.RDATA")
